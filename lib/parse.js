...
module.exports = function (dep, name, baseUrl) {
  ...
  function parsePaths(dep, name, baseUrl) {
    ...
    function getResolvedCssPaths() {
      if (! Array.isArray(main)) {
        return false;
      }

      var depcss = filterCss(pkgMain);

      var resolvedPaths = {};
      var resolve = resolver(resolvedPaths);
      _.each(depcss, resolve);
    ...
    function filterCss(arr) {
      var cssfiles = _.filter(arr, function (val) {
        return path.extname(val) === '.css';
      });
    ...
    var resolvedCssPaths = getResolvedCssPaths();

    if (resolvedCssPaths) {
        configElement.css = resolvedCssPaths;
    }
